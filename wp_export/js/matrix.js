...
function initMatrixForm(form) {
    ...
    form.addEventListener('submit', function (e) {
        ...
        // WP SPECIFIC: ADJUST URL
        // If your matrix results page is at /matrix-result/, use that.
        // Or if it's the same page with query params.
        // Assuming /matrix-result/ for now.
        window.location.href = `matrix-result?date=${encodeURIComponent(isoDate)}`;
    });
}
...
function initMatrixResult() {
    ...
    const setCell = (selector, val) => {
        ...
        if (el) el.childNodes[0].nodeValue = val;
    };
    ...
    // We are generating pure SVG, so it is safe to just clear and rebuild
    ...
    // Axes
    connectNodes(0, 4, outerPoints); connectNodes(2, 6, outerPoints);
    ...
    for (let i = 0; i < 8; i++) {
        ...
        let fill = (i % 2 !== 0) ? "#fff" : "#3366CC";
        let txt = (i % 2 !== 0) ? "#000" : "#fff";
    ...
    function drawRay(idx, col, txt, isFlip) {
        ...
        line.setAttribute("x2", pInner.x); line.setAttribute("y2", pInner.y);
    ...
    // Perimeter (Simplified for brevity, using same logic as source)
    // NOTE: The user has full implementation in source, copying logic for standard perimeter rendering
    function drawPerimeter(i1, i2, v1, v2, config) {
        ...
        const p2_ = reduce(p4 + v1), p1_ = reduce(p2_ + v1);
        const p3 = reduce(p2_ + p4);
        const p6 = reduce(p4 + v2), p5 = reduce(p4 + p6), p7 = reduce(p6 + v2);
        const pVals = [null, p1_, p2_, p3, p4, p5, p6, p7];
        ...
        for (let j = 1; j <= 7; j++) {
            ...
            l.setAttribute("x", tx + (config.shifts?.[j]?.x || -9)); l.setAttribute("y", ty + (config.shifts?.[j]?.y || -1));
            l.setAttribute("text-anchor", "middle"); l.setAttribute("font-size", j === 4 ? 11 : 9);
            l.textContent = pVals[j];
            textLayer.appendChild(l);
            if (config.years?.[j]) {
                const yLb = document.createElementNS("http://www.w3.org/2000/svg", "text");
                yLb.setAttribute("x", tx + (config.yearShifts?.[j]?.x || 5)); yLb.setAttribute("y", ty + (config.yearShifts?.[j]?.y || 8));
    ...
    drawPerimeter(0, 1, values[0], values[1], { shifts: { 1: { x: -9, y: -1 }, 2: { x: -9, y: -1 }, 3: { x: -9, y: -1 }, 4: { x: -12, y: -1 }, 5: { x: -9, y: -1 }, 6: { x: -9, y: -1 }, 7: { x: -9, y: -1 } }, years: { 1: "1–2", 2: "2–3", 3: "3–4", 4: "5 лет", 5: "6–7", 6: "7–8", 7: "8–9" }, yearShifts: { 1: { x: 5, y: 6 }, 2: { x: 5, y: 7 }, 3: { x: 5, y: 8 }, 4: { x: 5, y: 8 }, 5: { x: 5, y: 8 }, 6: { x: 5, y: 7 }, 7: { x: 5, y: 6 } } });
    drawPerimeter(1, 2, values[1], values[2], { shifts: { 1: { x: -9, y: -6 }, 2: { x: -9, y: -6 }, 3: { x: -9, y: -6 }, 4: { x: -9, y: -9 }, 5: { x: -9, y: -6 }, 6: { x: -9, y: -7 }, 7: { x: -9, y: -6 } }, years: { 1: "11–12", 2: "12–13", 3: "13–14", 4: "15 лет", 5: "16–17", 6: "17–18", 7: "18–19" }, yearShifts: { 1: { x: 1, y: 10 }, 2: { x: 1, y: 10 }, 3: { x: 1, y: 10 }, 4: { x: 1, y: 13 }, 5: { x: 1, y: 10 }, 6: { x: 1, y: 10 }, 7: { x: 1, y: 10 } } });
    drawPerimeter(2, 3, values[2], values[3], { shifts: { 1: { x: 8, y: -7 }, 2: { x: 8, y: -7 }, 3: { x: 8, y: -7 }, 4: { x: 12, y: -7 }, 5: { x: 8, y: -7 }, 6: { x: 8, y: -7 }, 7: { x: 5, y: -7 } }, years: { 1: "21–22", 2: "22–23", 3: "23–24", 4: "25 лет", 5: "26–27", 6: "27–28", 7: "28–29" }, yearShifts: { 1: { x: -17, y: 10 }, 2: { x: -17, y: 10 }, 3: { x: -17, y: 10 }, 4: { x: -20, y: 12 }, 5: { x: -17, y: 10 }, 6: { x: -17, y: 10 }, 7: { x: -17, y: 10 } } });
    drawPerimeter(3, 4, values[3], values[4], { shifts: { 1: { x: 9, y: 3 }, 2: { x: 9, y: 1 }, 3: { x: 9, y: 1 }, 4: { x: 12, y: 1 }, 5: { x: 9, y: 1 }, 6: { x: 9, y: 1 }, 7: { x: 9, y: -3 } }, years: { 1: "31–32", 2: "32–33", 3: "33–34", 4: "35 лет", 5: "36–37", 6: "37–38", 7: "38–39" }, yearShifts: { 1: { x: -24, y: 5 }, 2: { x: -24, y: 5 }, 3: { x: -24, y: 5 }, 4: { x: -32, y: 5 }, 5: { x: -24, y: 5 }, 6: { x: -24, y: 5 }, 7: { x: -24, y: 5 } } });
    drawPerimeter(4, 5, values[4], values[5], { shifts: { 1: { x: 9, y: 15 }, 2: { x: 9, y: 15 }, 3: { x: 9, y: 15 }, 4: { x: 12, y: 15 }, 5: { x: 9, y: 15 }, 6: { x: 9, y: 15 }, 7: { x: 9, y: 5 } }, years: { 1: "41–42", 2: "42–43", 3: "43–44", 4: "45 лет", 5: "46–47", 6: "47–48", 7: "48–49" }, yearShifts: { 1: { x: -25, y: 2 }, 2: { x: -25, y: 2 }, 3: { x: -25, y: 2 }, 4: { x: -33, y: 2 }, 5: { x: -25, y: 2 }, 6: { x: -25, y: 2 }, 7: { x: -25, y: 2 } } });
    drawPerimeter(5, 6, values[5], values[6], { shifts: { 1: { x: 9, y: 15 }, 2: { x: 9, y: 15 }, 3: { x: 9, y: 15 }, 4: { x: 12, y: 15 }, 5: { x: 9, y: 15 }, 6: { x: 9, y: 15 }, 7: { x: 9, y: 15 } }, years: { 1: "51–52", 2: "52–53", 3: "53–54", 4: "55 лет", 5: "56–57", 6: "57–58", 7: "58–59" }, yearShifts: { 1: { x: -15, y: -5 }, 2: { x: -15, y: -5 }, 3: { x: -15, y: -5 }, 4: { x: -20, y: -8 }, 5: { x: -15, y: -5 }, 6: { x: -15, y: -5 }, 7: { x: -15, y: -5 } } });
    drawPerimeter(6, 7, values[6], values[7], { shifts: { 1: { x: -9, y: 15 }, 2: { x: -9, y: 15 }, 3: { x: -9, y: 15 }, 4: { x: -12, y: 15 }, 5: { x: -9, y: 15 }, 6: { x: -9, y: 15 }, 7: { x: -9, y: 15 } }, years: { 1: "61–62", 2: "62–63", 3: "63–64", 4: "65 лет", 5: "66–67", 6: "67–68", 7: "68–69" }, yearShifts: { 1: { x: -2, y: -5 }, 2: { x: -2, y: -5 }, 3: { x: -2, y: -5 }, 4: { x: -2, y: -8 }, 5: { x: -2, y: -5 }, 6: { x: -2, y: -5 }, 7: { x: -2, y: -5 } } });
    drawPerimeter(7, 0, values[7], values[0], { shifts: { 1: { x: -9, y: 5 }, 2: { x: -9, y: 10 }, 3: { x: -9, y: 10 }, 4: { x: -12, y: 10 }, 5: { x: -9, y: 10 }, 6: { x: -9, y: 10 }, 7: { x: -9, y: 10 } }, years: { 1: "71–72", 2: "72–73", 3: "73–74", 4: "75 лет", 5: "76–77", 6: "77–78", 7: "78–79" }, yearShifts: { 1: { x: 5, y: 2 }, 2: { x: 5, y: 2 }, 3: { x: 5, y: 2 }, 4: { x: 5, y: 2 }, 5: { x: 5, y: 2 }, 6: { x: 5, y: 2 }, 7: { x: 5, y: 2 } } });

    // Extra
    const innerA = reduce(U[4] + U[6]), innerB = reduce(U[4] + innerA), innerC = reduce(U[6] + innerA);
    function drawExtra(angleIdx, offX, offY, val, letter, lOffX, lOffY, col, dol, hrt) {
        const ang = angles[angleIdx], rad = innerRadius2 * 0.5;
        const x = cx + rad * Math.cos(ang) + offX, y = cy + rad * Math.sin(ang) + offY;
        drawNode(x, y, 12, col, "#000", val, "#000", 14);
        const lc = document.createElementNS("http://www.w3.org/2000/svg", "circle");
        lc.setAttribute("cx", x + lOffX); lc.setAttribute("cy", y + lOffY); lc.setAttribute("r", 7); lc.setAttribute("fill", "#000");
        const lt = document.createElementNS("http://www.w3.org/2000/svg", "text");
        lt.setAttribute("x", x + lOffX); lt.setAttribute("y", y + lOffY); lt.setAttribute("text-anchor", "middle"); lt.setAttribute("dominant-baseline", "central");
        lt.setAttribute("fill", "#fff"); lt.setAttribute("font-size", 9); lt.setAttribute("font-weight", "bold"); lt.textContent = letter;
        nodeLayer.appendChild(lc); textLayer.appendChild(lt);
        if (dol) {
            const d = document.createElementNS("http://www.w3.org/2000/svg", "text");
            d.setAttribute("x", x - 15); d.setAttribute("y", y - 37); d.setAttribute("font-size", 26); d.setAttribute("fill", "#04dd00"); d.setAttribute("font-weight", "bold"); d.textContent = "$"; textLayer.appendChild(d);
        }
        if (hrt) {
            const hx = x - 35, hy = y - 35;
            const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
            path.setAttribute("d", `M ${hx} ${hy} c -5 -5, -15 0, -10 10 c 5 10, 15 10, 20 0 c 5 -10, -5 -15, -10 -10 Z`);
            path.setAttribute("fill", "#e84e42"); path.setAttribute("stroke", "#000"); nodeLayer.appendChild(path);
        }
    }
    ...
    // Mids
    const midRad = innerRadius2 / 2;
    drawNode(cx + midRad * Math.cos(angles[0]), cy + midRad * Math.sin(angles[0]), 15, "#73b55f", "#000", midU1, "#fff", 14);
    drawNode(cx + midRad * Math.cos(angles[2]), cy + midRad * Math.sin(angles[2]), 15, "#73b55f", "#000", midU2, "#fff", 14);

    // Markers on Outer
    function drawOuterMarker(i, letter, txt, ox, oy, tAlign, tOx, tOy) {
        const p = outerPoints[i]; const x = p.x + ox, y = p.y + oy;
        const c = document.createElementNS("http://www.w3.org/2000/svg", "circle");
        ...
        t.setAttribute("fill", "#fff"); t.setAttribute("font-weight", "bold"); t.setAttribute("font-size", 14); t.textContent = letter; textLayer.appendChild(t);
        const sub = document.createElementNS("http://www.w3.org/2000/svg", "text");
        sub.setAttribute("x", x + tOx); sub.setAttribute("y", y + tOy); sub.setAttribute("text-anchor", tAlign); sub.setAttribute("dominant-baseline", "central");
        sub.setAttribute("font-size", 13); sub.setAttribute("font-weight", "bold"); sub.setAttribute("fill", "#000"); sub.textContent = txt; textLayer.appendChild(sub);
    }
